...
\title{Using Normalizing Flows for AIPW Estimators with Continuous Treatment}
...
To estimate the causal effect of interest, e.g., ATE or ATT, a variety of methods have been proposed under the above unconfoundedness and overlap assumptions, referred as strong ignorability by \citet{rosenbaum1983central}. \citet{imbens2024lalonde} summarise these methods into three groups.
...
To improve this small sample performance, scholars have developed various mixed methods that combine outcome modeling with methods addressing covariate imbalance to eliminate remaining biases or improve precision. \citet{abadie2011biascorrecteda} provide the example that while the bias of a simple matching estimator might dominate variance in high-dimensional cases, adding regression to account for the remaining imbalance can substantially reduce such biases. The AIPW estimator for ATE is as the following:
